...
public:
    ...
    void printReverse();
    bool retrieve(string key, Type& object);
};

template <class Type>
bool unorderedLinkedList < Type >::retrieve(string key, Type& object)
{
    ...
    {
        if (current->info.key == key) //searchItem is found
        {
            object = current->info;
            return true;
        }
...
template < class Type >
void unorderedLinkedList < Type > ::printReverse()
{
    nodeType<Type>* current;
    for (int loc = this->count - 1; loc >= 0; loc--)
    {
        current = this->first;
        for (int i = 0; i < loc; i++)
